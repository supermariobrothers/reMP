...
LONG WINAPI exc_handler(_EXCEPTION_POINTERS* exc_inf)
{
	...
	// Write directly to samp_crash.log
	FILE *f = fopen("samp_crash.log", "w");
	if (f) {
		fputs(szErrorString, f);
		fclose(f);
	}

	LogDebug("CRASH OCCURRED! EIP: 0x%08X, Code: 0x%08X",
		(DWORD)exc_inf->ExceptionRecord->ExceptionAddress,
		exc_inf->ExceptionRecord->ExceptionCode);

	// Show cursor and minimize main window
	if (pGame && pGame->GetMainWindowHwnd()) {
		ShowWindow(pGame->GetMainWindowHwnd(), SW_MINIMIZE);
	}
	ShowCursor(TRUE);

	// Format readable crash message
	char szMsg[4096];
	_snprintf(szMsg, sizeof(szMsg) - 1,
		...
		"ESI: 0x%08X  EDI: 0x%08X  EBP: 0x%08X  ESP: 0x%08X\n"
		"EFLAGS: 0x%08X\n\n"
		"A crash log has been saved to 'samp_crash.log'.",
		pContextRecord->Eip,
		exc_inf->ExceptionRecord->ExceptionCode,
		pContextRecord->Eax, pContextRecord->Ebx, pContextRecord->Ecx, pContextRecord->Edx,
		pContextRecord->Esi, pContextRecord->Edi, pContextRecord->Ebp, pContextRecord->Esp,
		pContextRecord->EFlags);
	szMsg[sizeof(szMsg) - 1] = '\0';

	MessageBoxA(NULL, szMsg, "SA-MP Exception Handler", MB_OK | MB_ICONERROR | MB_TASKMODAL | MB_TOPMOST);

	ExitProcess(exc_inf->ExceptionRecord->ExceptionCode);

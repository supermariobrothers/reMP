...
//-----------------------------------------------------------
// Check if a specific ped model ID is valid and loadable

BOOL __stdcall IsValidPedModel(int iModelID)
{
	if (iModelID < 0 || iModelID > 20000) return FALSE;
	DWORD* dwModelArray = (DWORD*)0xA9B0C8;
	if (!dwModelArray) return FALSE;

	DWORD dwModelInfo = dwModelArray[ iModelID ];
	if (!dwModelInfo) {
		LogDebug("IsValidPedModel(%d): dwModelInfo is NULL", iModelID);
		return FALSE;
	}

	DWORD dwVtable = *(DWORD*)dwModelInfo;
	if (dwVtable != 0x85BDC0) {
		LogDebug("IsValidPedModel(%d): vtable=0x%08X (expected 0x85BDC0)", iModelID, dwVtable);
		return FALSE;
	}

	if (*(DWORD*)(dwModelInfo + 0x1C) != 0) return TRUE;

	DWORD cdSize = *(DWORD*)(0x8E4CCC + iModelID * 0x14);
	if (cdSize == 0) {
		LogDebug("IsValidPedModel(%d): cdSize=0", iModelID);
		return FALSE;
	}
...
{
	DWORD *dwModelInfos = (DWORD *)0xA9B0C8;
	return (BYTE *)dwModelInfos[nModelIndex];
}
...
{
	DWORD *dwModelInfos = (DWORD *)0xA9B0C8;
	BYTE *pModelInfoStart = (BYTE *)dwModelInfos[nModelIndex];
...
void _stdcall _VectorNormalise(VECTOR *vec)
{
	_asm mov ecx, vec
...
{
	c2->X = (short)(vec->X * 16383.5f);
	c2->Y = (short)(vec->Y * 16383.5f);
	c2->Z = (short)(vec->Z * 16383.5f);	
}
...
{
	vec->X = (float)c2->X;
	vec->X = (float)(vec->X / 16383.5f);
	vec->Y = (float)c2->Y;
	vec->Y = (float)(vec->Y / 16383.5f);
	vec->Z = (float)c2->Z;
	vec->Z = (float)(vec->Z / 16383.5f);	
}
...
void SaveCameraRaster(char *filename)
{
	...
	if(CamFrameBuffer) {
		WriteRwRasterToFile(CamFrameBuffer,"Raster2.bmp");
		pChatWindow->AddDebugMessage("Width %u Height: %u Depth: %u",Width,Height,Depth);
	}    
}

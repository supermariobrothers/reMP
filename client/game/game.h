...
public:	
	...
	CPlayerPed *NewPlayer(int iSkin,float fPosX,float fPosY,float fPosZ,float fRotation,BYTE byteCreateMarker = 1);
	...
	CActorPed *NewActor(int iSkin, float fPosX, float fPosY, float fPosZ, float fRotation);
	...
	void	SetCursorMode(int nMode, BOOL bImmediatelyHideCursor = FALSE);
	int		GetCursorMode();
	...
	void ToggleEnterExits(bool bEnabled);

	void LoadObjectInstance(char *szIPLLine);

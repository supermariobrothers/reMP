...
{
	LogDebug("CEntity::SetModelIndex(%u) called, m_pEntity=0x%X", uiModel, m_pEntity);
	if(!m_pEntity) return;

	if(!pGame->IsModelLoaded(uiModel)) {
		LogDebug("Model %u not loaded, requesting...", uiModel);
		pGame->RequestModel(uiModel);
		pGame->LoadRequestedModels();
		int iWait = 0;
		while(!pGame->IsModelLoaded(uiModel) && iWait < 100) {
			Sleep(5);
			iWait++;
		}
		LogDebug("Model %u load wait finished: iWait=%d, loaded=%d", uiModel, iWait, pGame->IsModelLoaded(uiModel));
	}

	if(!pGame->IsModelLoaded(uiModel)) {
		LogDebug("WARNING: Model %u failed to load after timeout, aborting SetModelIndex", uiModel);
		return;
	}

	DWORD dwThisEntity = (DWORD)m_pEntity;
	LogDebug("Calling internal GTA SetModelIndex for entity 0x%08X, model %u...", dwThisEntity, uiModel);
	...
	__try {
		...
		LogDebug("Internal GTA SetModelIndex succeeded for model %u", uiModel);
	}
	__except(EXCEPTION_EXECUTE_HANDLER) {
		LogDebug("CRASH caught in internal GTA SetModelIndex for model %u!", uiModel);
	}

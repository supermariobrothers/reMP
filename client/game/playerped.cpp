...
{
	DWORD dwPedPtr = (DWORD)m_pPed;
	LogDebug("CPlayerPed::SetModelIndex(%u): GTAId=%u, dwPedPtr=0x%08X", uiModel, m_dwGTAId, dwPedPtr);
	if(!GamePool_Ped_GetAt(m_dwGTAId)) {
		LogDebug("CPlayerPed::SetModelIndex: GamePool_Ped_GetAt(%u) returned NULL", m_dwGTAId);
		return;
	}
	if(!IsValidPedModel(uiModel)) {
		LogDebug("CPlayerPed::SetModelIndex: Model %u is not a valid ped model, defaulting to 0", uiModel);
		uiModel = 0;
	}
	...
	if(dwPedPtr) {
		...
		*(BYTE *)0x5A82C0 = 0xC3;
		...
		__try {
			...
			_asm call ebx
			LogDebug("CPlayerPed::SetModelIndex: Reset Ped Audio Attributes succeeded");
		}
		__except(EXCEPTION_EXECUTE_HANDLER) {
			LogDebug("CPlayerPed::SetModelIndex: EXCEPTION resetting Ped Audio Attributes");
		}
	}
}
...
void CPlayerPed::SetInterior(BYTE byteID)
{
	...
	*(BYTE*)0x00B72914 = byteID; // CWorld::ms_nCurrentArea
	*(BYTE*)((DWORD)m_pPed + 0x2F) = byteID; // entity.m_nAreaCode

...
BYTE byteUsedPlayerSlots[PLAYER_PED_SLOTS];
...
CPlayerPed *CGame::NewPlayer(int iSkin, float fPosX, float fPosY,
							 float fPosZ, float fRotation, BYTE byteCreateMarker)
{
	BYTE bytePlayerNum = FindFirstFreePlayerPedSlot();
	if(!bytePlayerNum) return NULL;
							 ...
							 float fPosZ, float fRotation, PCHAR szNumberPlate)
{
	...
	CVehicle *pVehicleNew = new	CVehicle(iType,fPosX,fPosY,fPosZ,fRotation,szNumberPlate,bKeepModelLoaded);
	//pChatWindow->AddDebugMessage("NewVehicle: %d",iType);
	return pVehicleNew;
}

//-----------------------------------------------------------

CActorPed *CGame::NewActor(int iSkin, float fPosX, float fPosY, float fPosZ, float fRotation)
{
	CActorPed *pActorNew = new CActorPed(iSkin,fPosX,fPosY,fPosZ,fRotation);
	return pActorNew;
}
							...
							float fPosZ, VECTOR vecRot)
{
	...
	if (pObjectNew && (!pObjectNew->m_dwGTAId || !pObjectNew->m_pEntity))
	{
		delete pObjectNew;
		return NULL;
	}
...
{
	if (iModelID < 0 || iModelID >= 20000) return;

	if (IsModelLoaded(iModelID)) return;

	DWORD* dwModelArray = (DWORD*)0xA9B0C8;
	if (dwModelArray && dwModelArray[iModelID] == 0) {
		LogDebug("RequestModel(%d): ModelInfo pointer is NULL, skipping", iModelID);
		return;
	}

	DWORD cdSize = *(DWORD*)(0x8E4CCC + iModelID * 0x14);
	if (cdSize == 0) {
		LogDebug("RequestModel(%d): Model has cdSize=0 (not in IMG), skipping request!", iModelID);
		return;
	}

	LogDebug("RequestModel(%d, %d): calling 0x4087E0...", iModelID, iLoadingStream);
	...
	LogDebug("RequestModel(%d): 0x4087E0 returned", iModelID);
}
...
{
	LogDebug("LoadRequestedModels(): calling 0x40EA10...");
	...
	__try {
		...
		LogDebug("LoadRequestedModels(): 0x40EA10 returned");
	}
	__except(EXCEPTION_EXECUTE_HANDLER) {
		LogDebug("LoadRequestedModels(): exception in 0x40EA10!");
	}
}

//-----------------------------------------------------------

int CGame::IsModelLoaded(int iModelID)
{
	if (iModelID < 0) return FALSE;
...
{
	//ScriptCommand(&toggle_radar_blank,iToggle);
...
void CGame::ToggleEnterExits(bool bEnabled)
{
	*(BYTE*)0x96A7C8 = (bEnabled) ? 0 : 1;
}

//-----------------------------------------------------------

void CGame::LoadObjectInstance(char *szIPLLine)
{
    _asm push szIPLLine
	_asm mov edx, 0x538690
	_asm call edx
	_asm pop ebx
}
...
void CGame::SetRequiredVehicleModels(BYTE *ModelCounts)
{
    ...
    // reset the array to hold the always loaded vehicles
	memset(&m_byteKeepLoadedVehicles[0],0,212);
    
	// remove any existing loaded models
	x=400; while(x!=612) { RemoveModel(x); x++; }

    // create a sorted array by times models used..

	// init the unsorted array which we'll sort by the passed ModelCounts
    int ModelListSorted[212];
	x=0; while(x!=212) { ModelListSorted[x] = x+400; x++; }

	// -- bubble sort
    int i=0,j=0,n=212;
	int iTempModelIndex;
	BYTE byteTempModelCount;

	for (i=0; i<n-1; i++) {
		for (j=0; j<n-1-i; j++) {
			if (ModelCounts[j+1] > ModelCounts[j]) {
				// swap
				byteTempModelCount = ModelCounts[j];
				iTempModelIndex = ModelListSorted[j];
				ModelCounts[j] = ModelCounts[j+1];
				ModelListSorted[j] = ModelListSorted[j+1];
				ModelCounts[j+1] = byteTempModelCount;
				ModelListSorted[j+1] = iTempModelIndex;
			}
		} // for inner
	} // for outer

	/* print off the indexes and their counts
	x=0; char tmpdbg[256];
	while(x!=212) {
		sprintf(tmpdbg,"VehicleModelIndex(%d,%u)\n",ModelListSorted[x],ModelCounts[x]);
		OutputDebugString(tmpdbg);
		x++;
	}*/

	// Keep the most commonly used vehicles loaded (up to 12, and only if count > 0)
	x=0;
	while(x < 12 && ModelCounts[x] > 0) {
		RequestModel(ModelListSorted[x]);
		m_byteKeepLoadedVehicles[ModelListSorted[x]-400]++;
		x++;
	}
...
void CGame::ForceFrameLimiterOn()
{
	// I can't see where it's being set
	// This will need to be called every frame.
	*(PBYTE)0xBA6794 = 1;    
}
//----------------------------------------------------
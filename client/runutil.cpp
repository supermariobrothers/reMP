...
void InitDebugLog()
{
	FILE *f = fopen("samp_debug.log", "w");
	if (f) {
		fprintf(f, "=== SA-MP Debug Log Initialized ===\n");
		fclose(f);
	}
}
...
void LogDebug(const char *fmt, ...)
{
	...
	va_list va;
	va_start(va, fmt);
	_vsnprintf(szBuffer, sizeof(szBuffer) - 1, fmt, va);
	va_end(va);
	szBuffer[sizeof(szBuffer) - 1] = '\0';

	fprintf(fLog, "[%02d:%02d:%02d.%03d] %s\n", st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, szBuffer);
	fflush(fLog);

	char szDbg[2200];
	_snprintf(szDbg, sizeof(szDbg) - 1, "[SA-MP] %s\n", szBuffer);

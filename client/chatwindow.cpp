...
CChatWindow::CChatWindow(IDirect3DDevice9 *pD3DDevice, ID3DXFont *pFont)
{
	...
	m_pD3DFont			= pFont;
	m_pD3DFontCE		= new ID3DXFontCE();
	m_pD3DFontCE->SetDXFont(m_pD3DFont);
	...
	m_dwChatDebugColor = D3DCOLOR_ARGB(255,190,190,190);
...
{
	char szClean[512];
	strncpy(szClean, sz, sizeof(szClean) - 1);
	szClean[sizeof(szClean) - 1] = '\0';
	RemoveColorEmbedsFromString(szClean);

	if(m_iEnabled == CHAT_WINDOW_MODE_FULL) {
		// above
		rect.top -= 1;
		m_pD3DFont->DrawText(m_pChatTextSprite,szClean,-1,&rect,DT_NOCLIP|DT_SINGLELINE|DT_LEFT,0xFF000000);
		rect.top += 1;
	...
	if (m_pD3DFontCE) {
		m_pD3DFontCE->DrawTextA(m_pChatTextSprite, sz, -1, &rect, DT_NOCLIP|DT_SINGLELINE|DT_LEFT, dwColor);
	} else {

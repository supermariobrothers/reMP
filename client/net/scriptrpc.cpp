...
void ScrSetSpawnInfo(RPCParameters *rpcParams)
{
	...
	memset(&SpawnInfo, 0, sizeof(PLAYER_SPAWN_INFO));

	RakNet::BitStream bsData((unsigned char*)Data, (iBitLength + 7) / 8, false);
	CPlayerPool *pPlayerPool = pNetGame->GetPlayerPool();

	int nBytes = iBitLength / 8;
	if (nBytes >= 46) {
		bsData.Read(SpawnInfo.byteTeam);
		bsData.Read(SpawnInfo.iSkin);
		bsData.Read(SpawnInfo.byteUnk);
		bsData.Read((char*)&SpawnInfo.vecPos, sizeof(VECTOR));
		bsData.Read(SpawnInfo.fRotation);
		bsData.Read((char*)SpawnInfo.iSpawnWeapons, sizeof(int) * 3);
		bsData.Read((char*)SpawnInfo.iSpawnWeaponsAmmo, sizeof(int) * 3);
	} else if (nBytes >= 45) {
		bsData.Read(SpawnInfo.byteTeam);
		bsData.Read(SpawnInfo.iSkin);
		SpawnInfo.byteUnk = 0;
	...
	if (pChatWindow) {
		pChatWindow->AddDebugMessage("[RPC] SetSpawnInfo: team=%d, skin=%d, pos=(%.1f, %.1f, %.1f)",
			SpawnInfo.byteTeam, SpawnInfo.iSkin, SpawnInfo.vecPos.X, SpawnInfo.vecPos.Y, SpawnInfo.vecPos.Z);
	}

	if (pPlayerPool && pPlayerPool->GetLocalPlayer()) {
...
void ScrSetPlayerPos(RPCParameters *rpcParams)
{
	...
	LogDebug("[RPC] ScrSetPlayerPos: (%.2f, %.2f, %.2f)", vecPos.X, vecPos.Y, vecPos.Z);
...
void ScrSetPlayerPosFindZ(RPCParameters *rpcParams)
{
	...
	LogDebug("[RPC] ScrSetPlayerPosFindZ: (%.2f, %.2f, %.2f)", vecPos.X, vecPos.Y, vecPos.Z);
...
void ScrSetInterior(RPCParameters *rpcParams)
{
	...
	LogDebug("[RPC] ScrSetInterior: %u", byteInterior);
	*(BYTE*)0x00B72914 = byteInterior; // CWorld::ms_nCurrentArea
...
void ScrSetCameraPos(RPCParameters *rpcParams)
{
	...
	bsData.Read(vecPos.Y);
	bsData.Read(vecPos.Z);
	LogDebug("[RPC] ScrSetCameraPos: (%.2f, %.2f, %.2f)", vecPos.X, vecPos.Y, vecPos.Z);
...
void ScrSetCameraLookAt(RPCParameters *rpcParams)
{
	...
	LogDebug("[RPC] ScrSetCameraLookAt: (%.2f, %.2f, %.2f), cut=%u", vecPos.X, vecPos.Y, vecPos.Z, byteCutType);
	if (pGame && pGame->GetCamera()) {
		pGame->GetCamera()->LookAtPoint(vecPos.X,vecPos.Y,vecPos.Z,byteCutType);	
	}
}
...
void ScrCreateObject(RPCParameters *rpcParams)
{
	...
	if (pObjectPool) {
		pObjectPool->New(wObjectID, iModel, vecPos, vecRot);
	}
}
...
void ScrDestroyObject(RPCParameters *rpcParams)
{
	...
	CObjectPool* pObjectPool =	pNetGame->GetObjectPool();
	if (pObjectPool && pObjectPool->GetAt(wObjectID))
	{
...
void ScrTogglePlayerSpectating(RPCParameters *rpcParams)
{
	...
	BOOL bToggle;
	bsData.Read(bToggle);
	LogDebug("[RPC] ScrTogglePlayerSpectating: bToggle=%d", bToggle);
	if (pPlayerPool && pPlayerPool->GetLocalPlayer()) {
...
void ScrForceSpawnSelection(RPCParameters *rpcParams)
{
	...
	if (pPlayerPool) {
		...
		if (pPlayer) pPlayer->ReturnToClassSelection();
	}
}
...
void ScrAttachObjectToPlayer(RPCParameters *rpcParams)
{
	...
	if (!pNetGame || !pNetGame->GetObjectPool() || !pNetGame->GetPlayerPool())
		return;
...
void ScrShowTextDraw(RPCParameters *rpcParams)
{
	...
	if (pTextDrawPool)
	{
		...
		memset(&TextDrawTransmit, 0, sizeof(TEXT_DRAW_TRANSMIT));
		...
		if (wTextLen > 0) {
			if (wTextLen >= MAX_TEXT_DRAW_LINE) {
				bsData.Read(cText, MAX_TEXT_DRAW_LINE - 1);
				cText[MAX_TEXT_DRAW_LINE - 1] = '\0';
			} else {
				bsData.Read(cText, wTextLen);
				cText[wTextLen] = '\0';
			}
		}

		char szDebug[256];
		sprintf(szDebug,"New TextDraw: %u (len: %u)\n", wTextID, wTextLen);

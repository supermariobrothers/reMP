...
BOOL CLocalPlayer::Spawn()
{
	...
	if (pGame) pGame->SetCursorMode(0, TRUE);
	if (pD3DDevice) pD3DDevice->ShowCursor(FALSE);
...
void CLocalPlayer::ReturnToClassSelection()
{
	m_bWantsAnotherClass = TRUE;
	m_bIsActive = FALSE;
	m_bWaitingForSpawnRequestReply = FALSE;
...
void CLocalPlayer::ProcessClassSelection(int iControlID)
{
	...
	else if(m_bClearedToSpawn) // WE ARE CLEARED TO SPAWN OR SELECT ANOTHER CLASS
	{
		...
		if (m_pPlayerPed) {
			...
			matPlayer.pos.X = 0.0f; matPlayer.pos.Y = 0.0f; matPlayer.pos.Z = 0.0f;
		}
		dwTicksSinceLastSelection = GetTickCount() - m_dwLastSpawnSelectionTick; // used to delay reselection.

		BOOL bLeftPressed = (iControlID == ID_CONTROL_LEFT) ||
			((GetAsyncKeyState(VK_LEFT) & 0x8000) != 0) ||
			((GetAsyncKeyState('A') & 0x8000) != 0);

		BOOL bRightPressed = (iControlID == ID_CONTROL_RIGHT) ||
			((GetAsyncKeyState(VK_RIGHT) & 0x8000) != 0) ||
			((GetAsyncKeyState('D') & 0x8000) != 0);

		// ALLOW ANOTHER SELECTION WITH THE LEFT KEY OR BUTTON
		if(bLeftPressed && (dwTicksSinceLastSelection > 250)) { // LEFT ARROW
			LogDebug("ProcessClassSelection: LEFT pressed, prev=%d, spawnsAvail=%d", m_iSelectedClass, pNetGame->m_iSpawnsAvailable);
			m_bClearedToSpawn = FALSE;
			m_dwLastSpawnSelectionTick = GetTickCount();
		...
		// ALLOW ANOTHER SELECTION WITH THE RIGHT KEY OR BUTTON
		else if(bRightPressed && (dwTicksSinceLastSelection > 250)) { // RIGHT ARROW
			LogDebug("ProcessClassSelection: RIGHT pressed, prev=%d, spawnsAvail=%d", m_iSelectedClass, pNetGame->m_iSpawnsAvailable);
			m_bClearedToSpawn = FALSE;
			m_dwLastSpawnSelectionTick = GetTickCount();
...
{
	LogDebug("CLocalPlayer::HandleClassSelection: requesting class %d", m_iSelectedClass);
	m_bClearedToSpawn = FALSE;
	m_bWaitingForSpawnRequestReply = FALSE;
	m_bIsActive = FALSE;
	if (pSpawnScreen) pSpawnScreen->ToggleVisibility(FALSE);
...
{
	LogDebug("CLocalPlayer::HandleClassSelectionOutcome: bOutcome=%d, skin=%d, pos=(%.1f, %.1f, %.1f)",
		bOutcome, m_SpawnInfo.iSkin, m_SpawnInfo.vecPos.X, m_SpawnInfo.vecPos.Y, m_SpawnInfo.vecPos.Z);
	if(bOutcome) {
		if(m_pPlayerPed) {
			m_pPlayerPed->ClearAllWeapons();
			LogDebug("HandleClassSelectionOutcome: setting skin %d...", m_SpawnInfo.iSkin);
			m_pPlayerPed->SetModelIndex(m_SpawnInfo.iSkin);
			LogDebug("HandleClassSelectionOutcome: skin set complete");
			m_pPlayerPed->TogglePlayerControllable(0);
		}
		m_bClearedToSpawn = TRUE;
		m_bWaitingForSpawnRequestReply = FALSE;
		if (!m_bIsSpectating) {
			LogDebug("HandleClassSelectionOutcome: showing spawn screen & cursor");
			if (pSpawnScreen) pSpawnScreen->ToggleVisibility(TRUE);
		}
	}
}
...
void CLocalPlayer::ProcessSpectating()
{
	...
	int iSendRate = 50;
	if (iSendRate < 30) iSendRate = 30;
	if((GetTickCount() - m_dwLastSendSpecTick) > (DWORD)iSendRate) {
		m_dwLastSendSpecTick = GetTickCount();
	...
	pGame->DisplayHud(FALSE);
	if (pSpawnScreen && pSpawnScreen->IsVisible()) pSpawnScreen->ToggleVisibility(FALSE);
	if (pD3DDevice) pD3DDevice->ShowCursor(FALSE);

	if (m_pPlayerPed) {
		m_pPlayerPed->SetHealth(100.0f);
		m_pPlayerPed->TeleportTo(spSync.vecPos.X, spSync.vecPos.Y, spSync.vecPos.Z + 20.0f);
	}
...
{
	LogDebug("CLocalPlayer::ToggleSpectating: bToggle=%d, cur_spectating=%d, cur_active=%d, wants_another=%d",
		bToggle, m_bIsSpectating, m_bIsActive, m_bWantsAnotherClass);

	if (m_bIsSpectating && !bToggle) {
		if (m_bIsActive && !m_bWantsAnotherClass) {
			LogDebug("ToggleSpectating: spawning existing active player");
			Spawn();
		} else {
			LogDebug("ToggleSpectating: resetting state and calling HandleClassSelection()");
			m_bIsActive = FALSE;
			m_bWantsAnotherClass = FALSE;
			m_bClearedToSpawn = FALSE;
			m_bWaitingForSpawnRequestReply = FALSE;

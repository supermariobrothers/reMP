...
{
	if (!p || !p->data || p->length == 0) return 255;

	if ((unsigned char)p->data[0] == ID_TIMESTAMP) {
		if (p->length <= sizeof(unsigned char) + sizeof(unsigned long)) return 255;
...
class CChatPacketLogger : public PluginInterface
{
public:
	...
	virtual PluginReceiveResult OnReceive(RakPeerInterface *peer, Packet *packet) { return RR_CONTINUE_PROCESSING; }
	...
	virtual void OnDirectSocketSend(const char *data, const unsigned bitsUsed, PlayerID remoteSystemID) {
		if (pChatWindow && data && bitsUsed >= 8) {
			pChatWindow->AddDebugMessage("[NET SND] ID: %d (%u bytes)", (unsigned char)data[0], (bitsUsed + 7) / 8);
		}
	}
	virtual void OnDirectSocketReceive(const char *data, const unsigned bitsUsed, PlayerID remoteSystemID) {
		if (pChatWindow && data && bitsUsed >= 8) {
			pChatWindow->AddDebugMessage("[NET RCV] ID: %d (%u bytes)", (unsigned char)data[0], (bitsUsed + 7) / 8);
		}
	}
...
static CChatPacketLogger s_ChatPacketLogger;
				   ...
				   PCHAR szPlayerName, PCHAR szPass)
{
	...
	m_pRakClient->AttachPlugin(&s_ChatPacketLogger);
...
{	
	...
	// KEEP THE FOLLOWING ANIMS AND BASE MODELS LOADED DURING THE NETGAME
	static bool bBaseModelsLoaded = false;
	if(!bBaseModelsLoaded && pGame->IsGameLoaded()) {
		if(!pGame->IsAnimationLoaded("PARACHUTE")) pGame->RequestAnimation("PARACHUTE");
		if(!pGame->IsModelLoaded(OBJECT_PARACHUTE)) pGame->RequestModel(OBJECT_PARACHUTE);
		if(!pGame->IsModelLoaded(WEAPON_MODEL_TEARGAS)) pGame->RequestModel(WEAPON_MODEL_TEARGAS);
		if(!pGame->IsModelLoaded(WEAPON_MODEL_GRENADE)) pGame->RequestModel(WEAPON_MODEL_GRENADE);
		if(!pGame->IsModelLoaded(WEAPON_MODEL_MOLTOV)) pGame->RequestModel(WEAPON_MODEL_MOLTOV);
		if(!pGame->IsModelLoaded(330)) pGame->RequestModel(330);
		pGame->LoadRequestedModels();
		bBaseModelsLoaded = true;
	}
...
//----------------------------------------------------
// AUTH KEY (Challenge / Response)

static void GenerateAuthKey(char* szAuthOut, const char* szAuthIn) {
	if (!szAuthOut || !szAuthIn) return;

	CSHA1 sha1;
	sha1.Update(reinterpret_cast<unsigned char*>(const_cast<char*>(szAuthIn)), static_cast<unsigned int>(strlen(szAuthIn)));
	sha1.Final();

	unsigned char buf[20];
	for (int k = 0; k < 5; ++k) {
		unsigned int dw = (static_cast<unsigned int>(sha1.m_digest[k * 4]) << 24) |
						  (static_cast<unsigned int>(sha1.m_digest[k * 4 + 1]) << 16) |
						  (static_cast<unsigned int>(sha1.m_digest[k * 4 + 2]) << 8) |
						  static_cast<unsigned int>(sha1.m_digest[k * 4 + 3]);
		memcpy(&buf[k * 4], &dw, 4);
	}

	static const unsigned char s_fallbackLogo[100] = {
		0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52, 
		0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x80, 0x08, 0x06, 0x00, 0x00, 0x00, 0xE4, 0xB5, 0xB7, 
		0x0A, 0x00, 0x00, 0x00, 0x09, 0x70, 0x48, 0x59, 0x73, 0x00, 0x00, 0x0B, 0x13, 0x00, 0x00, 0x0B, 
	...
	const unsigned char* pAuthBytes = s_fallbackLogo;

	for (int i = 0; i < 5; ++i) {
		unsigned char dl = buf[i];
		for (int j = 0; j < 100; ++j) dl = pAuthBytes[j] ^ dl ^ 0x2F;
		buf[i] = dl;
	}
	for (int i = 5; i < 10; ++i) {
		unsigned char dl = buf[i];
		for (int j = 0; j < 100; ++j) dl = pAuthBytes[j] ^ dl ^ 0x45;
		buf[i] = dl;
	}
	for (int i = 10; i < 15; ++i) {
		unsigned char dl = buf[i];
		for (int j = 0; j < 100; ++j) dl = pAuthBytes[j] ^ dl ^ 0x6F;
		buf[i] = dl;
	}
	for (int i = 15; i < 20; ++i) {
		unsigned char dl = buf[i];
		for (int j = 0; j < 100; ++j) dl = pAuthBytes[j] ^ dl ^ 0xDB;
		buf[i] = dl;
	}

	const unsigned char* pGtaBytes = reinterpret_cast<const unsigned char*>(0x004D3AA0);
	static const unsigned char s_fallbackGta[20] = {
		0x8B, 0x44, 0x24, 0x0C, 0x8B, 0x15, 0x34, 0xEA, 0xB4, 0x00,
		0x56, 0x57, 0x50, 0x8B, 0x44, 0x24, 0x14, 0x8D, 0x0C, 0x80
	};
	unsigned char gtaLocal[20];
	if (IsBadReadPtr(pGtaBytes, 20)) {
		memcpy(gtaLocal, s_fallbackGta, 20);
	} else {
		memcpy(gtaLocal, pGtaBytes, 20);
	}

	for (int i = 0; i < 20; ++i) {
		buf[i] ^= gtaLocal[i];
		sprintf(&szAuthOut[i * 2], "%02X", buf[i]);
	}
	szAuthOut[40] = '\0';
}
...
void CNetGame::Packet_AuthKey(Packet* p)
{
	...
	BYTE bytePacketId = 0;
	BYTE byteKeyLen = 0;
	bs.Read(bytePacketId);
	bs.Read(byteKeyLen);

	char szAuthIn[260] = { 0 };
	if (byteKeyLen > 0 && byteKeyLen < 256) {
		bs.Read(szAuthIn, byteKeyLen);
		szAuthIn[byteKeyLen] = '\0';
	}

	if (pChatWindow) {
		pChatWindow->AddDebugMessage("[NET AUTH] Server challenge: %s", szAuthIn);
	}

	char szAuthOut[260] = { 0 };
	bool bFound = false;
	for (int i = 0; i < 512; i++) {
		if (strcmp(AuthKeyTable[i][0], szAuthIn) == 0) {
			strcpy(szAuthOut, AuthKeyTable[i][1]);
			bFound = true;
			break;
		}
	}
	...
	if (!bFound) {
		GenerateAuthKey(szAuthOut, szAuthIn);
	}

	if (pChatWindow) {
		pChatWindow->AddDebugMessage("[NET AUTH] Auth response: %s", szAuthOut);
	}

	BYTE byteOutLen = (BYTE)strlen(szAuthOut);
	RakNet::BitStream bsOut;
	BYTE byteAuthPacketId = ID_AUTH_KEY;
	bsOut.Write(byteAuthPacketId);
	bsOut.Write(byteOutLen);
	bsOut.Write(szAuthOut, byteOutLen);
	m_pRakClient->Send(&bsOut, HIGH_PRIORITY, RELIABLE, 0);
}
...
void CNetGame::Packet_ConnectionSucceeded(Packet *p)
{
	...
	PCHAR szNickName = m_pPlayerPool ? m_pPlayerPool->GetLocalPlayerName() : "Jos";
	...
	bsSend.Write(szNickName, byteNameLen);

...
void InitGame(RPCParameters *rpcParams)
{
	...
	RakNet::BitStream bsInitGame((unsigned char*)Data, (iBitLength + 7) / 8, false);
	CPlayerPool *pPlayerPool = pNetGame->GetPlayerPool();
	...
	pNetGame->m_bZoneNames = bsInitGame.ReadBit();
	pNetGame->m_bUseCJWalk = bsInitGame.ReadBit();
	pNetGame->m_bAllowWeapons = bsInitGame.ReadBit();
	pNetGame->m_bLimitGlobalChatRadius = bsInitGame.ReadBit();
	bsInitGame.ReadBits((unsigned char*)&pNetGame->m_fGlobalChatRadius, 32);
	bStuntBonus = bsInitGame.ReadBit();
...
void RequestClass(RPCParameters *rpcParams)
{
	...
	if (pChatWindow) {
		char szHexDump[128] = { 0 };
		int dumpLen = min(nBytes, 16);
		for (int i = 0; i < dumpLen; i++) {
			char szByte[8];
			sprintf(szByte, "%02X ", (unsigned char)Data[i]);
			strcat(szHexDump, szByte);
		}
		pChatWindow->AddDebugMessage("[RPC] RequestClass bits=%d bytes=%d: %s", iBitLength, nBytes, szHexDump);
	}
	...
	LogDebug("[RPC] RequestClass: outcome=%d, team=%d, skin=%d, pos=(%.1f, %.1f, %.1f)",
		byteRequestOutcome, SpawnInfo.byteTeam, SpawnInfo.iSkin, SpawnInfo.vecPos.X, SpawnInfo.vecPos.Y, SpawnInfo.vecPos.Z);

	if (pChatWindow) {
		pChatWindow->AddDebugMessage("[RPC] RequestClass: outcome=%d, team=%d, skin=%d, pos=(%.1f, %.1f, %.1f)",
			byteRequestOutcome, SpawnInfo.byteTeam, SpawnInfo.iSkin, SpawnInfo.vecPos.X, SpawnInfo.vecPos.Y, SpawnInfo.vecPos.Z);
	}
	...
	if (pPlayer) {
		if (byteRequestOutcome) {
			...
			pPlayer->m_bClearedToSpawn = FALSE;
		}
	}
}

		...
		switch(dwChar) {
			...
			case 'c':
				if (pNetGame && pNetGame->GetPlayerPool()) {
					...
					CVehicle* pVehicle = pNetGame->GetVehiclePool()->GetAt(pPlayer->m_CurrentVehicle);
					if (pVehicle)
						pVehicle->ToggleLights( (pVehicle->AreLightsEnabled() == false) );
				}
				return TRUE;
		}
	}
...
{ 	
	...
	if(pSpawnScreen && pSpawnScreen->IsVisible()) {
		if (pSpawnScreen->MsgProc(hwnd, uMsg, wParam, lParam)) {
			return 0;
		}
	}

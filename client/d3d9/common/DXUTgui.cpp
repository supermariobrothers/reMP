...
{   
    ...
    if( m_pManager->m_pStateBlock ) m_pManager->m_pStateBlock->Capture();
    ...
    if( pTextureNode ) pd3dDevice->SetTexture( 0, pTextureNode->pTexture );

    if( m_pManager->m_pSprite ) m_pManager->m_pSprite->Begin( D3DXSPRITE_DONOTSAVESTATE );
    ...
    if( m_pManager->m_pSprite ) m_pManager->m_pSprite->End();

    if( m_pManager->m_pStateBlock ) m_pManager->m_pStateBlock->Apply();
